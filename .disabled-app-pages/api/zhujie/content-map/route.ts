...
import { normalizeText, splitLines, hashContent } from '@zhujie/lib/hash';
import { detectMetadata } from '@zhujie/lib/metadata';
import { getContent, storeContent, ensureTables } from '@zhujie/lib/db';
import { generateContentMap } from '@zhujie/lib/annotate';
import type { ContentMapRequest, ContentMapResponse } from '@zhujie/lib/types';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const existing = await getContent(hash);
    if (!existing) {
      return NextResponse.json({ error: 'Not found' }, { status: 404 });
    }
    const lines = splitLines(existing.source_text);
