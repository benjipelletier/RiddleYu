// scripts/generate.js — run locally to generate and cache a puzzle
// Usage: node scripts/generate.js [YYYY-MM-DD] [--force-update]
//
// --force-update: if puzzle exists, re-run riddle generation only (keeps same chengyus)
//                 if no puzzle exists, runs full generation
// default date: today (America/New_York)
...
async function main() {
  console.log(`Generating puzzle for ${date}${forceUpdate ? ' (force-update)' : ''}...`)

  // Check for existing puzzle
  const existing = await kv.get(`puzzle:${date}`)
  if (existing && !forceUpdate) {
    console.log(`Puzzle for ${date} already exists. Use --force-update to re-generate riddles.`)
    process.exit(0)
  }

  // Fetch used 成语 to avoid duplicates
  const usedChengyu = (await kv.get('used_chengyu')) || []

  let structure
  if (existing && forceUpdate) {
    // Re-use existing chengyus — only regenerate riddles
    console.log(`Puzzle for ${date} exists. Keeping chengyus, regenerating riddles...`)
    console.log(`  Keeping: ${existing.chengyus.map(cy => cy.chars.join('')).join(', ')} → ${existing.hidden.chars.join('')}`)
    structure = existing
  } else {
    // Full generation: pick new chengyus
    console.log(`Avoiding ${usedChengyu.length} already-used 成语`)
    console.log('Call 1: Generating puzzle structure...')
    ...
    structure = JSON.parse(structureMsg.content[0].text.trim())
  ...
  console.log('Call 2: Generating riddle translations and hints...')
  ...
  // Record all 5 成语 as used (skip if we reused existing chengyus — they're already tracked)
  if (!existing) {
    const newEntries = [
  ...
  console.log(`Done! Puzzle for ${date}:`)
  console.log(`  Solved: ${puzzle.chengyus.map(cy => cy.chars.join('')).join(', ')}`)
  console.log(`  Hidden: ${puzzle.hidden.chars.join('')}`)
}
